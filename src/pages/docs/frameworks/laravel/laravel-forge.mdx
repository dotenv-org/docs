...
<FrameworksBreadcrumb framework='Laravel' url='/docs/frameworks/laravel' icon={
  <>
    ...
    <LaravelForgeIcon className="h-10 w-10 text-[#212936]"/>
  </>
} />
...
Install [Laraval](https://laravel.com/docs) and create a new Laravel project.
...
Require [`phpdotenv-vault`](https://github.com/dotenv-org/phpdotenv-vault) in your `composer.json` file.
...
Modify Laravel's bootstrap script to use `phpdotenv-vault`. This is necessary to later decrypt the `.env.vault` file and inject environment variables just in time.

```php {{ title: 'bootstrap/app.php' }}
<?php
// At the top of bootstrap/app.php
...
```
<Note>
  I experienced strange behavior with Laravel Forge where I needed to call safeLoad twice to make sure the load happened. I was unable to solve why. If you know why or don't experience this issue, please let me know [@motdotla](https://twitter.com/@motdotla).
</Note>
